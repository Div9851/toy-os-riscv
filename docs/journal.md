...
- **(i-4) `sys_write` を実装**: D0027 の方針に沿って `write(fd, buf, len)` を POSIX semantics で実装。`copyin(pt, dst_kernel, va_user, len)` を `walk` ベースで書き、fd=1 のみ UART に書き出し、それ以外は `-EBADF` (= -9)。`init.rs` を `write(1, b"hello\n", 6)` に書き換えて `putc` を撤去。errno 定数 `EBADF` を syscall.rs に追加。
- (i 完了後) シェル到達への次のマイルストーン: スケジューラ + fork + exec syscall + 簡易 FS。順序は要検討。
