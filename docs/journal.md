...
1. **(a) Console を 16550 UART 直叩きで実装、モジュール化** (D0012, D0013) — `src/uart.rs` に `Uart16550`、`src/console.rs` に `static CONSOLE: spin::Mutex<Uart16550>` と `print!`/`println!` を配置。`src/memlayout.rs` に `KERNBASE` / `PHYSTOP` / `UART0` / `PLIC` / `CLINT` / `VIRTIO0` を定数化。`linker.ld` に `__kernel_end = .;` を追加。SBI Console は撤去。
2. **(c) トラップ・割り込みハンドラの最低限** — `stvec` 設定 / トラップフレーム保存 / `scause`/`sepc`/`stval` ダンプ。あわせて Console の Mutex に `push_off`/`pop_off` (ロック区間中の割り込み禁止) を導入。panic ハンドラの lockless 経路もここで整える。
3. **(e) 物理ページアロケータ** — `[__kernel_end, PHYSTOP)` を 4 KiB ページ単位で freelist 管理。
4. **(f) Sv39 ページングを identity map で有効化** — カーネル RAM + MMIO を identity マップしたカーネルページテーブルを組み、`satp.MODE = 8` で切り替え。`sfence.vma` で TLB をフラッシュ。
5. **(g) ユーザページテーブル + 最初の U-mode 遷移** — init 用のページテーブルを組み、`sstatus.SPP = 0` / `sepc = entry` を設定して `sret` で U-mode へ。
6. **(h) syscall ABI の雛形** — `ecall` を `scause = 8` としてハンドル。最低限 `write` / `exit` のみ。
7. **(i) init を埋め込み ELF としてロード** — `include_bytes!` で取り込んだ ELF をパースしてユーザページテーブルにマップ、`sret` でエントリへ。

UART RX 割り込みによるキーボード入力は (c) の後に別途。スケジューラ・`fork` は (i) が動いてから (D0009)。
