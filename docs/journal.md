...
- **(c'-2) `push_off` / `pop_off` + 自前 Spinlock の導入**。`Cpu { noff, intena }` をシングルコア前提で `static` に 1 個置き、自前 Spinlock の acquire/release と連動させる。spin crate の `Mutex` は撤去予定。Console を新 Spinlock に切り替えて、割り込みハンドラから `println!` を呼んでも安全な状態にする (D として記録予定)。
- **(c'-3) PLIC + UART RX**。priority/threshold/enable/claim/complete のサイクルを通し、UART の IER で受信割り込みを有効化、キー入力を `kerneltrap` 経由で受け取る。
